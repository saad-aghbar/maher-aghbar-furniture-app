...
export function OrderBasketProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch('/api/basket', { credentials: 'same-origin' });
        if (res.ok) {
          const body = (await res.json()) as { lines?: BasketLine[] };
          if (!cancelled && Array.isArray(body.lines) && body.lines.length) {
            setLinesState(body.lines.map((line) => emptyBasketLine(line)));
            setHydrated(true);
