    ...
    <div className="maher-stagger grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {tiles.map((tile, i) => (
        <div key={tile.key} className={featuredFirst && i === 0 ? 'sm:col-span-2' : undefined}>
          <Link href={tileLink(tile.href, tile.filter)} className="block">
            <FloorBoard
              accent={
                tile.count > 0 && /block|late|overdue|fail|attention/i.test(tile.key)
                  ? 'warning'
                  : 'brand'
              }
            >
              <p className="text-sm text-[var(--maher-text-secondary)]">{labelFor(tile)}</p>
              <p className="mt-2 text-2xl font-semibold tabular-nums" dir="ltr">
                <AnimatedValue value={tile.count} enabled />
              </p>
            </FloorBoard>
          </Link>
        </div>
      ...
      {steps.map((step, i) => {
        ...
        return (
          <Link
            ...
            className="maher-press relative z-[1] flex min-w-[100px] flex-1 flex-col items-center gap-1 overflow-hidden rounded-[20px] border border-[var(--maher-border-strong)] bg-[var(--maher-surface)] px-2.5 py-3 text-center shadow-[var(--maher-shadow-sm)]"
    ...
    <div className="space-y-8 pb-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {data.attention.slice(0, 6).map((card) => (
              <Link
                ...
                className="block"
              >
                <FloorBoard accent={card.priority === 'critical' ? 'error' : card.priority === 'high' ? 'warning' : 'brand'}>
                  <div className="flex items-start justify-between gap-2">
                    <p className="font-semibold text-[var(--maher-text-primary)]">{copy.attentionTitle(card)}</p>
                    <ArrowUpRight className="h-4 w-4 shrink-0 text-[var(--maher-brand)]" />
                  </div>
                  <p className="mt-1 text-sm text-[var(--maher-text-secondary)]">{copy.attentionWhy(card)}</p>
                  <p className="mt-2 text-xs font-semibold text-[var(--maher-brand)]">{copy.attentionAction(card)}</p>
                </FloorBoard>
              </Link>
            ))}
          </div>
          {data.late?.overdue ? (
            <Link href={tileLink(data.late.overdue.href, data.late.overdue.filter)} className="block">
              <FloorBoard accent={data.late.overdue.count > 0 ? 'error' : 'success'}>
                <p className="text-sm text-[var(--maher-text-secondary)]">{labelFor(data.late.overdue)}</p>
                <p className="mt-2 text-2xl font-semibold tabular-nums" dir="ltr">
                  <AnimatedValue value={data.late.overdue.count} enabled />
                </p>
              </FloorBoard>
            </Link>
          ) : null}
      ...
      {data.finance ? (
        <SectionShell title={tCommon('mgmtSectionMoney')} hint={tCommon('mgmtSectionMoneyHint')}>
          <div className="space-y-4">
            ...
            <Link href={tileLink(data.finance.openInvoices.href, data.finance.openInvoices.filter)} className="block">
              <FloorBoard>
                <p className="text-sm text-[var(--maher-text-secondary)]">{labelFor(data.finance.openInvoices)}</p>
                <p className="mt-2 text-2xl font-semibold tabular-nums" dir="ltr">
                  <AnimatedValue value={data.finance.openInvoices.count} enabled />
                </p>
                <p className="mt-1 text-sm tabular-nums" dir="ltr">
                  {money(data.finance.receivable, currency)}
                </p>
              </FloorBoard>
            </Link>
      ...
      <section className="space-y-3">
        ...
        <div className="maher-stagger flex flex-wrap gap-2">
          {(
            [
              { href: '/admin/production', label: tNav('production'), icon: Factory },
              { href: '/admin/inventory', label: tNav('inventory'), icon: Boxes },
              { href: '/admin/deliveries', label: tNav('deliveries'), icon: Truck },
              { href: '/admin/quality', label: tNav('quality'), icon: PackageCheck },
              { href: '/admin/returns', label: tNav('returns'), icon: RotateCcw },
              { href: '/admin/reports', label: tNav('reports'), icon: ClipboardList },
