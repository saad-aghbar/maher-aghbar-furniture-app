...
/** Browser calls go same-origin so auth cookies land on :3000, not :4000. */
const REQUEST_ORIGIN = typeof window === 'undefined' ? API_URL : '';
...
async function requestOnce(path: string, init?: RequestInit): Promise<Response> {
  return fetch(`${REQUEST_ORIGIN}${path}`, {
    ...init,
    credentials: 'include',
    headers: {
...
export async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (res.status === 401 && !path.includes('/auth/refresh') && !path.includes('/auth/login')) {
    const { refreshWebSession } = await import('./refresh-session');
