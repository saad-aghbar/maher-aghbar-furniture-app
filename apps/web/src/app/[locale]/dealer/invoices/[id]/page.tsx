...
export default function InvoiceDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6">
      <BackButton fallbackHref="/dealer/invoices" />
      <PageHero
        tone="soft"
        title={inv.number}
      ...
      <Card>
        ...
        {inv.salesOrder ? (
          <p className="mt-3 text-sm">
            <Link href={`/dealer/orders/${inv.salesOrder.id}`} className="text-brand hover:underline">
              {inv.salesOrder.number}
            </Link>
          </p>
        ) : null}
        <Link href="/dealer/payments" className="mt-3 inline-block text-sm text-brand hover:underline">
          {t('payments')}
        </Link>
      </Card>
      {inv.lines?.length ? (
