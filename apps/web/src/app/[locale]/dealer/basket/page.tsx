  ...
  return (
    <div className="space-y-6">
      ...
      <PageHero
        ...
        actions={
          <Button variant="secondary" onClick={() => router.push('/dealer/order/custom')}>
            {t('customItem')}
          </Button>
        }
      />
      ...
      {lines.length === 0 ? (
        <EmptyState
          ...
          description={tc('basketEmptyHint')}
          action={
            <Button onClick={() => router.push('/dealer/catalog')}>{t('catalog')}</Button>
          }
        />
      ) : (
            ...
            return (
              <Card key={line.id} className="flex gap-4">
                ...
                <div className="flex flex-col items-end gap-2">
                  {line.productId ? (
                    <Link
                      href={`/dealer/catalog/${line.productId}/customize?variantId=${line.variantId}&qty=${line.quantity}&lineId=${line.id}`}
