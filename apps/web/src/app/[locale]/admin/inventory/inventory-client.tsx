...
export default function InventoryPage() {
  ...
  const orderMaterialsMutation = useMutation({
    ...
    onSuccess: async (created) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['inventory-low-stock'] });
      if (created?.id) router.push(`/admin/purchasing/requests/${created.id}`);
    },
    onError: (err) => setActionError(mutationErrorMessage(err)),
  });
            ...
            <div className="flex flex-wrap gap-2">
              {inspectLot.salesOrder?.id ? (
                <Button
                  ...
                  onClick={() => {
                    setInspectLot(null);
                    router.push(`/admin/sales-orders/${inspectLot.salesOrder!.id}`);
                  }}
                >
                  {tl('viewOrder')}
                </Button>
              ...
              <li key={row.id}>
                <button
                  ...
                  onClick={() => {
                    ...
                    router.push(`/admin/purchasing/${row.id}`);
                  }}
                >
                  <span>
