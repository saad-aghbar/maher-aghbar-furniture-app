  ...
  return (
    <div className="space-y-6">
      <PageHero
        ...
        tone="soft"
        actions={
          <Link href="/admin/reports/returns" className="text-sm underline">
            {ta('lensReturns')}
          </Link>
        }
      />
      ...
      {d.salesOrder ? (
        <Link href={`/admin/sales-orders/${d.salesOrder.id}`} className="text-sm underline">
          {d.salesOrder.number}
        </Link>
      ) : null}
      <Card className="p-4">
          ...
          <TableBody>
            {(d.pieces ?? []).map((piece, index) => (
              <TableRow key={piece.id}>
                <TableCell>
                  <Link href={`/admin/returns/${d.id}`} className="underline">
                    {index + 1}
                  </Link>
                </TableCell>
