                  ...
                  <TableRow key={item.id}>
                    <TableCell>
                      <div className="flex flex-wrap items-center gap-2">
                        <Link href={`/admin/requests/${params.id}/lines/${item.id}`} className="text-brand hover:underline">
                          {item.productName}
                        </Link>
                        {item.variantLabel ? (
        ...
        <Card title={tc('quotations')}>
          <ul className="space-y-2">
            {data.quotations!.map((q) => (
              <li key={q.id}>
                <Link href={`/admin/quotations/${q.id}`} className="font-medium text-brand hover:underline">
                  {q.number}
                </Link>{' '}
