...
export type InventoryHomeSection = 'items' | 'transfers' | 'counts';

const SECTIONS: InventoryHomeSection[] = ['items', 'transfers', 'counts'];
...
type Props = {
  active: InventoryHomeSection;
  onChange: (section: InventoryHomeSection) => void;
};
...
/**
 * Floor section switcher — Fabric bubble; tap or hold-and-scrub like production chips.
 */
export function InventorySectionTabs({ active, onChange }: Props) {
  const { t, isRTL, locale } = useLocale();
  const { colors, colorScheme } = useTheme();
  const reduce = useReducedMotion();
  const [layouts, setLayouts] = useState<Partial<Record<InventoryHomeSection, ChipLayout>>>(
    {},
  );
  ...
  const onChipLayout = useCallback((name: InventoryHomeSection, e: LayoutChangeEvent) => {
    ...
    <GestureDetector gesture={gesture}>
      ...
      >
        ...
        {SECTIONS.map((section) => {
          ...
          const label = t(`mobile.inventory.sections.${section}`);
          const ink = accentFor(section, colors, focused);
