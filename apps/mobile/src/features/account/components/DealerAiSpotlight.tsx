...
/** Featured AI assistant banner — gradient grid + drifting orbs (dealer Account hub). */
export function DealerAiSpotlight() {
  const { user } = useAuth();
  ...
  const shellProps = reduce
    ? {}
    : { entering: FadeInDown.delay(240).duration(400).damping(22) };
  ...
  return (
    <Shell {...shellProps} style={{ gap: theme.spacing.sm }}>
      ...
      >
        {t('mobile.dealerAccount.aiEyebrow')}
      </AppText>

      <AnimatedPressable
        variant="card"
        accessibilityRole="button"
        accessibilityLabel={t('mobile.dealerAccount.aiTitle')}
        onPress={() => {
          void haptics.confirmMedium();
          router.push('/(app)/(customer)/ai-chat' as Href);
        }}
        style={{
        ...
        >
          ...
          >
            <AppText
              ...
              numberOfLines={1}
            >
              {t('mobile.dealerAccount.aiTitle')}
            </AppText>
            <AppText
              variant="caption"
              weight="regular"
