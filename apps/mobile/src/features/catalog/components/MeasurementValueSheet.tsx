...
export function MeasurementValuePanel({
  ...
  const [draftUnit, setDraftUnit] = useState(unit || 'cm');
  ...
  useEffect(() => {
    if (!active) return;
    const preset = MEASUREMENT_UNIT_PRESETS.includes(
      unit as MeasurementUnitPreset,
    );
    setDraftUnit(unit || 'cm');
  ...
  const unitOk = resolvedUnit.length > 0;
  ...
  const apply = () => {
    ...
    onSelect(formatted, resolvedUnit);
  };
