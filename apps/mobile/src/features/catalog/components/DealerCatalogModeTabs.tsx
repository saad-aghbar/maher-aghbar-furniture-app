...
/** Warm parchment stops — all → favorites → ordered. */
const FILL_LIGHT = ['#F5F1EA', '#F3EEE5', '#F0EBE3'] as const;
const BORDER_LIGHT = ['#C4BDB0', '#B8A88A', '#8F7A58'] as const;
const FILL_DARK = [
  'rgba(168,144,108,0.22)',
  'rgba(181,164,140,0.20)',
  'rgba(168,144,108,0.18)',
] as const;
const BORDER_DARK = ['#A8906C', '#B5A48C', '#A8906C'] as const;

type ChipLayout = { x: number; width: number };

/**
 * All / Favorites / Ordered — Fabric touch bar (tap or hold-and-scrub).
 */
export function DealerCatalogModeTabs({ value, onChange }: Props) {
  ...
  const [layouts, setLayouts] = useState<Partial<Record<CatalogBrowseMode, ChipLayout>>>(
    {},
  );

  const activeIdx = Math.max(0, CATALOG_BROWSE_MODES.indexOf(value));

  const orderedLayouts = useMemo(
    () => CATALOG_BROWSE_MODES.map((mode) => layouts[mode]),
    [layouts],
  );

  const onSelectIndex = useCallback(
    (index: number) => {
      const next = CATALOG_BROWSE_MODES[index];
  ...
  const onChipLayout = useCallback((name: CatalogBrowseMode, e: LayoutChangeEvent) => {
