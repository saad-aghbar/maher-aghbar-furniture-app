 ...
 * Cross-level deps are bridged via the nearest intermediate on the path when possible,
 * otherwise kept as a single long edge (layout still spaces rows).
 * Never fan-out a parent to every node on the next row (that creates spaghetti).
 */
export function displayStageEdges(layout: StageGraphLayout): StageGraphEdge[] {
  ...
  // Direct adjacent-level edges from the real DAG.
  ...
  // Bridge longer spans without full fan-out: attach parent to the child-side
  // neighbor on the next row that best continues toward the real target.
  for (const edge of layout.edges) {
    const fromLevel = levelOf.get(edge.from);
    const toLevel = levelOf.get(edge.to);
    if (fromLevel == null || toLevel == null) continue;
    ...
    const candidates = layout.nodes.filter((n) => n.level === nextLevel);
    ...
    // Prefer a next-level node that is an ancestor of the target (on a path).
 ...
 */
function orderLanesByBarycenter(
  levels: Map<number, OrderStageView[]>,
  levelCount: number,
  edges: StageGraphEdge[],
): Map<number, OrderStageView[]> {
  ...
  for (let pass = 0; pass < 4; pass += 1) {
    // Top-down
    const nextDown = new Map<number, OrderStageView[]>();
    ...
    // Bottom-up
    const nextUp = new Map<number, OrderStageView[]>();
    for (let level = levelCount - 1; level >= 0; level -= 1) {
      ...
      const lanes = laneOf();
      // Refresh lane map from current ordered for lower levels already written
    ...
    // Merge: keep top-down for level 0, use bottom-up for rest from this pass
    const merged = new Map<number, OrderStageView[]>();
    for (let level = 0; level < levelCount; level += 1) {
 ...
 * Lanes are barycenter-ordered to cut crossings on dense graphs.
 */
