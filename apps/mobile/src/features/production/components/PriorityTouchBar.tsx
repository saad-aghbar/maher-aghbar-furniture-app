...
type PriorityTouchBarProps = {
  value: ProductionPriority | string;
  onChange: (next: ProductionPriority) => void;
};

function priorityIndex(value: string): number {
  const i = PRIORITIES.indexOf(value as ProductionPriority);
  return i >= 0 ? i : 1; // NORMAL
}
...
/**
 * Priority scrub bar — Fabric bubble, color per level, tap or hold-and-drag.
 */
export function PriorityTouchBar({ value, onChange }: PriorityTouchBarProps) {
  const { t, isRTL, locale } = useLocale();
  const { colors, colorScheme } = useTheme();
  const reduce = useReducedMotion();
  const [layouts, setLayouts] = useState<Partial<Record<ProductionPriority, ChipLayout>>>(
    {},
  );
  ...
  const onChipLayout = useCallback((name: ProductionPriority, e: LayoutChangeEvent) => {
    ...
    <GestureDetector gesture={gesture}>
      ...
      >
        ...
        {PRIORITIES.map((p) => {
          ...
          const label = t(key);
          const display = label === key ? p : label;
