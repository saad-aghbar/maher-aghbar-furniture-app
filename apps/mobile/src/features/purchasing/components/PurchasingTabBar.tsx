...
type Props = {
  tabs: TabItem[];
  value: PurchasingHubTab;
  onChange: (tab: PurchasingHubTab) => void;
};
...
export function PurchasingTabBar({ tabs, value, onChange }: Props) {
  ...
  const onChipLayout = useCallback((name: PurchasingHubTab, e: LayoutChangeEvent) => {
  ...
  const colorStops = useMemo(() => {
    // interpolateColor needs evenly spaced input matching visible length
      ...
      <GestureDetector gesture={gesture}>
        ...
        >
          ...
          {visible.map((item) => {
            ...
            const shortKey = `mobile.purchasing.tabs.${item.key}`;
