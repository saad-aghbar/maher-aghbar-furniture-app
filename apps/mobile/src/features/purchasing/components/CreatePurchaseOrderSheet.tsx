...
export function CreatePurchaseOrderSheet({ open, onClose, onCreated }: Props) {
  ...
  const submit = async () => {
    ...
    if (lines.some((l) => Number(l.quantity) <= 0 || !Number.isFinite(Number(l.unitCost)))) {
    ...
    try {
      ...
      showToast({ variant: 'success', message: t('catalog.purchaseOrderCreated') });
      dismiss();
      onCreated(po.id);
    } catch (err) {
      void haptics.error();
  ...
  return (
    <>
      <BottomSheet
        open={open}
        onClose={dismiss}
        title={t('catalog.newPurchaseOrder')}
        ...
        <View style={{ gap: theme.spacing.md, flex: 1, minHeight: 0 }}>
          ...
          >
            ...
            >
              ...
              <SecondaryButton
                label={t('catalog.newSupplier')}
                onPress={() => setCreateSupplierOpen(true)}
                ...
                >
                  ...
                  >
                    <AppText variant="caption" color="muted" dir="ltr">
                      {formatCurrency(lineTotal(line.quantity, line.unitCost))}
                    </AppText>
                    <AnimatedPressable
                      variant="button"
                      onPress={() =>
            ...
            >
              <TotalsRow label={t('mobile.purchasing.subtotal')} value={formatCurrency(totals.subtotal)} />
              <TotalsRow label={t('mobile.purchasing.tax')} value={formatCurrency(totals.tax)} />
              <TotalsRow
