...
type Props = {
  ...
  onChange: (next: ReturnStatusFilter) => void;
};
...
export function ReturnsStatusRail({ value, onChange }: Props) {
  ...
  const [layouts, setLayouts] = useState<Partial<Record<ReturnStatusFilter, ChipLayout>>>(
    {},
  );

  const activeIdx = Math.max(0, RETURN_STATUS_FILTERS.indexOf(value));

  const orderedLayouts = useMemo(
    () => RETURN_STATUS_FILTERS.map((segment) => layouts[segment]),
    [layouts],
  );

  const onSelectIndex = useCallback(
    (index: number) => {
      const next = RETURN_STATUS_FILTERS[index];
  ...
  const onChipLayout = useCallback((name: ReturnStatusFilter, e: LayoutChangeEvent) => {
