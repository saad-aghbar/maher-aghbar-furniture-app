...
import type { DealerQuoteDesk } from '../dealerQuotationUi';

const DESKS: DealerQuoteDesk[] = ['all', 'action', 'accepted', 'closed'];

const LABEL_KEY: Record<DealerQuoteDesk, string> = {
  all: 'mobile.dealerQuotations.deskAll',
  action: 'mobile.dealerQuotations.deskAction',
  accepted: 'mobile.dealerQuotations.deskAccepted',
  closed: 'mobile.dealerQuotations.deskClosed',
};
...
/** First four stops of the users wood → olive set. */
...
type Props = {
  value: DealerQuoteDesk;
  onChange: (next: DealerQuoteDesk) => void;
};

/** All / Review / Accepted / Closed wood bubble for the dealer quote desk. */
export function QuotationDeskRail({ value, onChange }: Props) {
  ...
  const [layouts, setLayouts] = useState<Partial<Record<DealerQuoteDesk, ChipLayout>>>({});
  const activeIdx = Math.max(0, DESKS.indexOf(value));

  const orderedLayouts = useMemo(
    () => DESKS.map((desk) => layouts[desk]),
    [layouts],
  );

  const onSelectIndex = useCallback(
    (index: number) => {
      const next = DESKS[index];
  ...
  const onChipLayout = useCallback((name: DealerQuoteDesk, e: LayoutChangeEvent) => {
  ...
  return (
    <GestureDetector gesture={gesture}>
      ...
      >
        ...
        />
        {DESKS.map((desk) => {
          const focused = desk === value;
          const label = t(LABEL_KEY[desk]);
          return (
            <Pressable
              key={desk}
              ...
              onLayout={(e) => onChipLayout(desk, e)}
              onPress={() => onSelectIndex(DESKS.indexOf(desk))}
              style={{
                flex: 1,
                height: PILL_HEIGHT,
                paddingHorizontal: 4,
