...
const TAB_ICON: Record<InvoiceDeskTab, keyof typeof Ionicons.glyphMap> = {
  all: 'layers-outline',
  orders: 'cube-outline',
...
type Props = {
  tabs: TabItem[];
  value: InvoiceDeskTab;
  onChange: (tab: InvoiceDeskTab) => void;
  /** Skip the outer board when the bar already sits inside invoice chrome. */
  embedded?: boolean;
};

export function InvoicesTabBar({ tabs, value, onChange, embedded }: Props) {
  ...
  const onChipLayout = useCallback((name: InvoiceDeskTab, e: LayoutChangeEvent) => {
  ...
  const colorStops = useMemo(() => {
    ...
    const border = visible.map((_, i) => borders[Math.min(i, borders.length - 1)]!);
