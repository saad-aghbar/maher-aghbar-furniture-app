...
import { filterRegistry, getAuditStats, getLabRegistry } from '../registry';
...
/**
 * Coverage / Registry — secondary Dev Tests surface (audit + technical list).
 * Visual Showroom lives at `/dev/tests`.
 */
export function DevTestsCoverageScreen() {
  ...
  return (
    <ScrollableScreen>
      <AppText variant="largeTitle" weight="semibold" testID="dev-tests-coverage-title">
        Coverage / Registry
      </AppText>
      <AppText variant="bodySecondary" color="secondary" style={{ marginBottom: theme.spacing.sm }}>
        Technical inventory & aesthetic work queue. Visual playground is Showroom.
      </AppText>

      <Pressable
        onPress={() => router.back()}
        style={{ marginBottom: theme.spacing.md, alignSelf: isRTL ? 'flex-end' : 'flex-start' }}
      >
      ...
      <TextField
        label="Search by file, component, path…"
        value={query}
        onChangeText={setQuery}
        autoCapitalize="none"
        autoCorrect={false}
        testID="dev-tests-coverage-search"
      />
