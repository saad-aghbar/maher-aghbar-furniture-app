...
export function BottomSheetTransition({
  ...
  useEffect(() => {
    ...
    if (progress >= 1 && drag > 0 && sheetHeight > 0) {
      // Fold the finger offset into progress so close continues from here.
      p.value = Math.min(1, drag / sheetHeight);
      offset.value = 0;
    }
  ...
  }, [durationMs, isDragging, offset, p, progress, reduce, sheetHeight]);

  const sheetStyle = useAnimatedStyle(() => {
    const drag = Math.max(0, offset.value);
    return {
      transform: [{ translateY: p.value * sheetHeight + drag }],
    };
  });

  const backdropAnim = useAnimatedStyle(() => {
    const drag = Math.max(0, offset.value);
    const combined = sheetHeight > 0 ? p.value + drag / sheetHeight : p.value;
