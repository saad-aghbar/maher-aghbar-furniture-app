  ...
  if (!timing) {
    return {
      ...
      progressPercent: liveTaskProgressPercent({
        status: task.status,
        elapsedMinutes: task.timing?.elapsedMinutes ?? 0,
...
export function workflowGraphHasRunningTimer(graph: {
  stages?: Array<{ running?: boolean; openStartedAt?: string | null }>;
}): boolean {
  return Boolean(graph.stages?.some((s) => s.running && s.openStartedAt));
}

export function withLiveWorkflowGraph<T extends {
...
}>(graph: T, now?: Date | number): T {
  const stages = graph.stages.map((stage) => {
    const elapsed = liveElapsedFromTimer(
      {
        running: stage.running,
        openStartedAt: stage.openStartedAt,
  ...
  return {
    ...graph,
    stages,
    progressPercent: liveWorkflowProgressPercent(stages),
  };
}
